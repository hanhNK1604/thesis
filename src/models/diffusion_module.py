...
class DiffusionModule(L.LightningModule): 
    ...
    def validation_step(self, batch, batch_index): 
        ...
        if batch_index == 16: 
            ...
            mask_resize = make_grid((mask_resize > 0.5).long(), nrow=2) 
            image_resize = make_grid(image_resize, nrow=2)
            
            self.logger.log_image(images=[mask_pred_n], key='val/mask_pred_n')
            self.logger.log_image(images=[mask_pred], key='val/mask_pred')
            self.logger.log_image(images=[mask_resize], key='val/mask')
            self.logger.log_image(images=[image_resize], key='val/image')
    ...
    def test_step(self, batch, batch_index): 
        ...
        mask_pred = self.diffusion_model.sample(image=image_resize_norm, cfg_scale=self.cfg_scale)
        iou = self.iou(mask_pred, (mask_resize > 0.5).long())
        self.log('test/iou', iou, on_epoch=True, on_step=False, prog_bar=True) 

        mask_pred_n = self.sample_n(image=image_resize_norm, n=5) 
        iou_n = self.iou(mask_pred_n, (mask_resize > 0.5).long()) 
        ...
        if batch_index == 16: 
            ...
            self.logger.log_image(images=[mask_resize], key='test/mask')
            self.logger.log_image(images=[image_resize], key='test/image')
